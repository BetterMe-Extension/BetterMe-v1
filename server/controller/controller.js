...
/* Import Statements */
const db = require("../db-models/db-models");
>>>>>>> 474b6e6c9b554f1bd53d6577e9a1de8cd46b2d32
...
appControllers.signup = async (req, res, next) => {
  ...
  await db.query(q, [username, email], async (err, data) => {
    ...
    if (result.rows.length > 0) {
      res.locals.error = 'Account with this username/email already exists. Please try with different username/email';
      next();
    } else {
      await db.query(
        ...
        async (err, data) => {
          ...
          await db.query('SELECT * FROM users WHERE username=($1)', [username], (err, data) => {

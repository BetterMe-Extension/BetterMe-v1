...
function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('searchFetch')) {
      ...
      axios
        .get('https://api.spoonacular.com/recipes/complexSearch?apiKey=b3dfd59b7c1f48a888d4c0c01d659bf1&number=50')
        .then((res) => {
  ...
  const createSearch = async () => {
    ...
    await axios
      .get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=b3dfd59b7c1f48a888d4c0c01d659bf1&number=50&query=${Search.current.value}`)
      .then((res) => {
        console.log(res.data);
        localStorage.setItem('searchFetch', JSON.stringify(res.data));
        // console.log(res.data)
      ...
      <div>
        <div className={style.Hero} style={{ backgroundImage: `url(${foodArr})` }}>
          ...
          <div className={style.Bar}>
            ...
            <Icon onClick={createSearch} className={style.search} path={mdiMagnify} size={1.5} />
          </div>
        </div>
      </div>
      <br />
